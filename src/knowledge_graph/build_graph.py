...
build_graph.py
==============
Entry point chạy toàn bộ pipeline XÂY DỰNG Legal Knowledge Graph — bước chạy
MỘT LẦN trên Google Colab (cần GPU để load Qwen2.5-7B trích xuất thực thể),
sinh ra `data/extracted_json/`.

Runtime của chatbot KHÔNG chạy file này: `scripts/run_neo4j_ingest.py` chỉ đọc
lại `data/extracted_json/` đã sinh sẵn rồi nạp vào Neo4j (xem service
`kg-ingest` trong docker-compose.yml).
...
4. Chạy: python -m src.knowledge_graph.build_graph --data-dir /path/to/data/keep
...
# Thêm thư mục gốc project vào sys.path để `from src....` chạy được khi gọi
# thẳng file này (python src/knowledge_graph/build_graph.py) trên Colab.
...
def step2_extract_entities(
    ...
    from src.knowledge_graph.entity_extractor import LegalEntityExtractor
    from src.knowledge_graph.graph_builder import get_van_ban_meta, get_van_ban_id
...
def step3_build_graph(
    ...
    from src.knowledge_graph.graph_builder import LegalGraphBuilder, get_van_ban_meta, get_van_ban_id
...
def main():
    ...
    if args.skip_extraction:
        ...
        from src.knowledge_graph.entity_extractor import load_llm
